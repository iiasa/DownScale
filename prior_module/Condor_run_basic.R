...
for (hostdom in hostdoms) {
  ...
  seed_job_template <- c(
    ...
    "",
    "periodic_remove = (JobStatus == 1) && (CurrentTime - EnteredCurrentStatus > 60 )", # if seed job remains idle for more than 2 minutes, remove it as presumably the execute host is not responding
    "",
    "request_memory = 0",
...
bat_template <- c(
  ...
  "@echo on",
  "net use H: \\\\hdrive.iiasa.ac.at\\home$\\u046\\wogerer",
...
job_template <- c(
  ...
  "",
  "notification = Never", # Per-job, so you'll get spammed setting it to Always or Complete.
  "",
